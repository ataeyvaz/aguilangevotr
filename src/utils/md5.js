/**
 * Pure-JS MD5 — Python hashlib.md5(text.encode('utf-8')).hexdigest() ile birebir eşleşir.
 * Bağımlılık yok.
 */
...
export function md5(str) {
  // Unicode → UTF-8 bytes (same as Python str.encode('utf-8'))
  // eslint-disable-next-line no-undef
  const bytes = new TextEncoder().encode(str)
  const l = bytes.length
  const nblks = ((l + 8) >> 6) + 1
  const blks = new Array(nblks * 16).fill(0)
  for (let i = 0; i < l; i++) blks[i >> 2] |= bytes[i] << ((i % 4) * 8)
  blks[l >> 2] |= 0x80 << ((l % 4) * 8)
  blks[nblks * 16 - 2] = l * 8
